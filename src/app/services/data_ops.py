...
def merge_dataframes(
    ...
    try:
        if use_similarity:
            ...
            # Use suffixes only if there are column conflicts
            on_cols = on if isinstance(on, list) else [on]
            df1_cols = set(df1.columns) - set(on_cols)
            df2_cols = set(df2.columns) - set(on_cols)
            
            # Only add suffixes if there are overlapping non-key columns
            if df1_cols & df2_cols:
                result = pd.merge(df1, df2, on=on, how=how, suffixes=('_x', '_y'))
            else:
...
def _merge_with_similarity(
    ...
    # Non-matching subsets for potential similarity merge (only on first key for simplicity)
    key = on_cols[0]
    if how in ["inner", "left"]:
        unmatched_df1 = df1[~df1[key].isin(df2[key])]
    else:
        unmatched_df1 = pd.DataFrame(columns=df1.columns)

    if how in ["inner", "right", "outer"]:
        unmatched_df2 = df2[~df2[key].isin(df1[key])]
    else:
        unmatched_df2 = pd.DataFrame(columns=df2.columns)
    ...
    if not unmatched_df1.empty and not unmatched_df2.empty:
        ...
        for idx1, row1 in unmatched_df1.iterrows():
            ...
            for idx2, row2 in unmatched_df2.iterrows():
                ...
                for col in similarity_columns:
                    if col in row1 and col in row2:
                        v1 = str(row1[col]) if pd.notna(row1[col]) else ""
                        v2 = str(row2[col]) if pd.notna(row2[col]) else ""
                        score = fuzz.token_set_ratio(v1, v2) / 100.0
                        scores.append(score)
            ...
            if best_match is not None:
                pairs.append((idx1, best_match, best_score))

        if pairs:
            # Build similarity DataFrame with suffixed columns to mirror pandas merge
            left_idx = [p[0] for p in pairs]
            right_idx = [p[1] for p in pairs]
            left = unmatched_df1.loc[left_idx].reset_index(drop=True)
            right = unmatched_df2.loc[right_idx].reset_index(drop=True)

            left_nonkey = [c for c in left.columns if c not in on_cols]
            right_nonkey = [c for c in right.columns if c not in on_cols]

            data: Dict[str, Any] = {}
            # Key columns come from left
            for k in on_cols:
                data[k] = left[k].values
            # Left non-key with _x suffix
            for c in left_nonkey:
                data[f"{c}_x"] = left[c].values
            # Right non-key with _y suffix
            for c in right_nonkey:
                data[f"{c}_y"] = right[c].values

            sim_merged = pd.DataFrame(data)

            # Align to exact_match columns if available
            if not exact_match.empty:
                # Ensure all columns exist
                for col in exact_match.columns:
                    if col not in sim_merged.columns:
                        sim_merged[col] = pd.NA
                sim_merged = sim_merged[exact_match.columns]
                result = pd.concat([exact_match, sim_merged], ignore_index=True)
            else:
                result = sim_merged
        else:
            result = exact_match
...
def detect_outliers(
    ...
    """
    Detect outliers using modified Z-score (median/MAD) or IQR method.

    Args:
        df: Input DataFrame
        columns: Numeric columns to analyze
        method: 'zscore' (modified z) or 'iqr'
        threshold: Z threshold (default 3.5) or IQR multiplier (default 1.5)
    ...
    for col in columns:
        ...
        if method == "zscore":
            # Modified Z-score using median and MAD (robust for small samples)
            median = np.median(series)
            mad = np.median(np.abs(series - median))
            if mad == 0:
                result_df[f"{col}_outlier"] = False
                continue
            mod_z = 0.6745 * (series - median) / mad
            result_df[f"{col}_outlier"] = np.abs(mod_z) > threshold
        else:
            ...
            result_df[f"{col}_outlier"] = (series < lower) | (series > upper)
